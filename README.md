...
> **R:** area_circulo aparece como U en programa.o porque solo está declarada y no definida en ese archivo, mientras que en matematica.o aparece como T porque ahí está su implementación real.
...
> **R:** Una macro como CUADRADO(x) se reemplaza en el preprocesamiento por texto, antes de compilar, y no tiene verificación de tipos.

Una función real como sumar(a, b) es evaluada en tiempo de ejecución, pasa por el compilador y sí tiene verificación de tipos y parámetros.

La macro desaparece en el preprocesamiento (-E), mientras que la función se mantiene hasta el enlazado/ejecución.
...
> **R:** T es un símbolo definido en la sección de código (text), o sea una función o instrucción ejecutable dentro del archivo objeto.

D es un símbolo definido en la sección de datos (data), o sea variables globales inicializadas con valor.

T vive en el segmento de código ejecutable y D en el segmento de datos del programa.
...
> **R:** ntdll.dll => /c/Windows/SYSTEM32/ntdll.dll (0x7ffde4bd0000)
KERNEL32.DLL => /c/Windows/System32/KERNEL32.DLL (0x7ffde4ac0000)
KERNELBASE.dll => /c/Windows/System32/KERNELBASE.dll (0x7ffde25e0000)
ucrtbase.dll => /c/Windows/System32/ucrtbase.dll (0x7ffde24e0000)

¿Por qué `libc` no hubo que especificarla explícitamente al enlazar con `gcc`?

> **R:** Porque gcc la enlaza automáticamente por defecto, ya que es la biblioteca estándar de C y contiene funciones esenciales como printf y exit.
